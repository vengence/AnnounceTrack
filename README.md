...
- **任务组织与诊断**：概览按分组折叠任务，侧栏任务可滚动；队列、日志和异常详情可直接查看。
- **可调采集范围**：支持固定每日检查时间、只检查最新 N 条、排除固定页脚文本，以及页面改版后重新生成方案并静默重建基线。
- **用量与存储**：设置页显示 DeepSeek 累计 tokens 和本地文件占用，并允许修改新公告摘要与版本对比提示词。
...
- 新公告会生成包含核心事项、适用对象、关键时间和动作要求的摘要后通知；摘要提示词可在设置中调整。
...
- 支持按列表项 ID 或标题执行一次详情点击；暂不自动执行多步搜索、滚动或复杂交互。
