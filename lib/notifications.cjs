...
async function sendWechat(webhook, task, event) {
  ...
  const typeLabel = eventTypeLabel(event.type, task);
  const eventUrl = safeHttpUrl(event.url);
  const lines = [
    `### ${escapeMarkdown(typeLabel)} · ${escapeMarkdown(task.name)}`,
    `> **${task.type === "page" ? "页面" : "公告"}：** ${escapeMarkdown(event.title || (task.type === "page" ? "未命名页面" : "未命名公告"))}`,
    event.summary ? `> **摘要：** ${escapeMarkdown(event.summary)}` : "",
    eventUrl ? `[查看原${task.type === "page" ? "页面" : "公告"}](${escapeMarkdownUrl(eventUrl)})` : "",
...
async function sendEmail(config, task, event, version) {
  ...
  const subject = `【${eventTypeLabel(event.type, task)}】${task.name} · ${event.title || (task.type === "page" ? "页面更新" : "公告更新")}`;
    ...
    <div style="max-width:820px;margin:0 auto;padding:32px 18px">
      <div style="background:#fff;border:1px solid #dfe4df;border-radius:14px;overflow:hidden">
        <div style="padding:26px 30px;border-bottom:1px solid #e5e9e5">
          <div style="font-size:12px;color:#69736c;margin-bottom:9px">${escapeHtml(task.name)} · ${escapeHtml(eventTypeLabel(event.type, task))}</div>
      ...
      <p style="text-align:center;color:#8a938c;font-size:12px">由监控助手于 ${formatLocal(event.createdAt)} 采集</p>
    </div>
  </body></html>`;
}
...
function buildEmailText(task, event, version) {
  ...
  return [
    `${task.name} · ${eventTypeLabel(event.type, task)}`,
    event.title,
...
function eventTypeLabel(type, task = {}) {
  if (task.type === "page" && type === "content_updated") return "页面内容更新";
  return ({
