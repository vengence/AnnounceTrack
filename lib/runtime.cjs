...
class AnnouncementRuntime {
  ...
  async runTask(taskId, reason) {
    ...
    try {
      ...
      const allItems = extractList(task.plan.list, listCapture, task.listUrl);
      const maxItems = Math.max(0, Math.min(100, Number(task.maxItems || 0)));
      ...
      const baseline = previous.length === 0 || Boolean(storedTask?.baselineResetPending);
      ...
      await this.store.update((data) => {
        ...
        if (currentTask) {
          ...
          currentTask.baselineResetPending = false;
          currentTask.nextRunAt = nextRunAt(currentTask.frequencyMinutes, resolvedQuietHours, currentTask.dailyTime);
          currentTask.stats = {
            ...(currentTask.stats || {}),
  ...
  async runPageTask(task, runId, resolvedQuietHours, alertPolicy) {
    ...
    let event = null;
    if (previousVersion && version && !task.baselineResetPending) {
  ...
  async processItem(task, item, triggerType, baseline, listCapture = null, detailStats = null) {
    ...
    let detail = null;
    if (task.monitorMode !== "list_only" && task.plan?.detail) {
      if (detailStats) detailStats.attempted += 1;
      if (!detailUrl) {
        if (detailStats) detailStats.failed += 1;
        this.emit("detail-capture-failed", { taskId: task.id, taskName: task.name, message: `详情采集失败：${item.title} · 列表项没有可执行的详情地址或交互规则` });
      }
    }
    if (task.monitorMode !== "list_only" && task.plan?.detail && detailUrl) {
      try {
        ...
        const capture = embedded && listCapture
          ? listCapture
          : await this.capture(detailUrl, task.plan.detail, task.plan?.relation?.detailInteraction === "click_item" ? "点击列表项加载详情" : embedded ? "同页详情" : "详情页", task, item);
        if (looksLikeExpiredLogin(task, capture)) throw authenticationExpiredError();
...
function nextRunAt(frequencyMinutes = 15, quietHours = null, dailyTime = "") {
  const frequency = Math.max(5, Math.min(7 * 24 * 60, Number(frequencyMinutes || 15)));
  if (frequency === 1440 && /^\d{2}:\d{2}$/.test(String(dailyTime || ""))) {
    const [hour, minute] = dailyTime.split(":").map(Number);
    const candidate = new Date();
    candidate.setHours(Math.min(23, hour), Math.min(59, minute), 0, 0);
