...
function extractDetail(plan, capture, item = null) {
  if (plan.sourceType === "dom_region") {
    ...
    const content = String(region.text || "").trim();
    if (content.length < 10) return null;
    return {
      title: region.title || capture.dom?.title || "页面内容",
      content: content.slice(0, MAX_DETAIL_TEXT_CHARS),
      html: sanitizeHtml(region.html || escapeHtml(content).replace(/\n/g, "<br>"), capture.finalUrl),
      createdAt: "",
      updatedAt: "",
      metadata: [],
  ...
  if (plan.sourceType === "dom_embedded") {
    ...
    return {
      ...
      content: content.slice(0, MAX_DETAIL_TEXT_CHARS),
      html: sanitizeHtml(detail.html || escapeHtml(content).replace(/\n/g, "<br>"), capture.finalUrl),
  ...
  if (plan.sourceType === "dom") {
    const content = String(capture.dom?.text || "").trim();
    return {
  ...
  for (const response of responses) {
    ...
    return {
      title,
      content: htmlToText(raw).slice(0, MAX_DETAIL_TEXT_CHARS),
      html: sanitizeHtml(raw, capture.finalUrl),
...
function resolveDetailUrl(task, item) {
  if (task.plan?.detail?.sourceType === "dom_embedded") return task.listUrl;
